...
// melding"). Telephony/quotes/email are enabled once their configuration
// exists (Phase 3b/3c-a) — until then, or if Exact stays unconfigured, this
// reads as an expected-state notice, not an error. Async because
// createEmailAdapter() reads MonitoredMailbox from the database (the other
// three adapters stay env-var-only and synchronous).
export async function AdapterStatusBanner() {
  ...
  const email = (await createEmailAdapter()).status();

  if (telephony.available && quotes.available && exact.available && email.available) return null;
  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-border-subtle bg-canvas px-3 py-2 text-xs text-ink-tertiary">
      ...
      <div className="space-y-0.5">
        ...
        {!email.available && <p>E-mailgeschiedenis is nog niet (volledig) gekoppeld.</p>}
      </div>
    </div>
  );
}
