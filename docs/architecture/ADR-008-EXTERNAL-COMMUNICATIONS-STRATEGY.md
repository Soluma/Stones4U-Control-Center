...
| `EMAIL_INBOUND`, `EMAIL_OUTBOUND` | Twee providers, provider-onafhankelijk achter één `EmailAdapter`: Microsoft Graph (`info@stones4u.nl`, tenant-breed app-credential) en IMAP (`info@stones4u.eu`, server-side credential) — zie `30-PHASE-3C-EMAIL-INTEGRATION-DISCOVERY.md`; eerdere versies van dit ADR gingen hier eerst onjuist uit van Gmail, daarna van Microsoft 365 als enige bron | Nooit — live geprojecteerd, geen berichttekst opgeslagen |
...
**Waarom uitsluitend B, geen A**: elk van deze bronnen blijft het systeem-van-waarheid voor zijn eigen data (Shopify voor orders, TelefoonSysteem voor gesprekken, Microsoft Graph/Exchange Online en het IMAP-mailboxsysteem voor e-mail, de offerte-apps voor offertes) — dit is een directe toepassing van het al vastgelegde principe in ADR-001/ADR-004 ("bestaande systemen blijven satellites, geen duplicatie zonder noodzaak") en vermijdt de bekende faalmodus die elders in het landschap al is aangetroffen: verouderde/gedupliceerde kopieën die uit sync raken met de bron (`docs/platform-discovery/14-SHARED-CORE-DESIGN.md`).

**Deduplicatie**: omdat niets lokaal wordt opgeslagen, is duplicatie **door constructie** onmogelijk — er is geen tweede rij die met een live-geprojecteerd item kan botsen. Elk geprojecteerd item krijgt een stabiele, samengestelde synthetische ID (`telefoon-call-{id}`, `m365-{mailboxId}-{messageId}`/`imap-{mailboxId}-{uidValidity}-{uid}` — provider-aware, zie `30-PHASE-3C-EMAIL-INTEGRATION-DISCOVERY.md` §8, `offerte-{bron}-{id}`, `shopify-draftorder-{gid}`) — zelfde patroon als het bestaande `shopify-order-{gid}`.
...
- Een cache wordt pas overwogen **nadat** live-latentie in de praktijk een probleem blijkt (geen vooraf-optimalisatie) — en dan uitsluitend als een kortlevende (minuten, niet uren/dagen), niet-persistente metadata-cache (nooit berichttekst/gespreksinhoud), per de richtlijn in `30-PHASE-3C-EMAIL-INTEGRATION-DISCOVERY.md` §5.
